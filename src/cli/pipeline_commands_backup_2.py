...
class PipelineCommand:
    ...
    def run_pipeline(
        ...
        try:
            ...
            # Apply limit to seed queries if specified
            if limit:
                # Calculate how many seed queries needed to generate 'limit' examples
                # Each seed generates 30 transformations (5 personas × 3 complexity × 2 tool)
                seeds_needed = max(1, (limit + 29) // 30)  # Round up
                original_count = len(seed_queries)
                seed_queries = seed_queries[:seeds_needed]
                print(f"✅ Loaded {original_count} seed queries (limiting to {len(seed_queries)} seed queries to generate ~{limit} examples)")
            else:
            ...
            # Apply final limit to transformations if specified
            if limit and len(all_transformations) > limit:
                all_transformations = all_transformations[:limit]
                print(f"\n⚙️  Limited to {len(all_transformations)} transformations for trajectory generation")
