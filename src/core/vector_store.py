...
FIXED VERSION: Includes deduplication to prevent adding identical chunks.
"""
...
class VectorStore:
    ...
    def add_chunks(
        ...
        INCLUDES DEDUPLICATION to prevent adding identical chunks.
        ...
        for i, chunk in enumerate(chunks):
            ...
            # DEDUPLICATION: Skip if we've seen similar text
            # Use first 200 chars as fingerprint (adjust if needed)
            text_fingerprint = text[:200].strip()
            if text_fingerprint in seen_texts:
                duplicates_removed += 1
                logger.debug(f"Skipping duplicate chunk {i} (fingerprint already seen)")
                continue
        ...
        if ids:
            ...
            logger.info(f"✅ Added {len(ids)} unique chunks to vector store")
        else:
